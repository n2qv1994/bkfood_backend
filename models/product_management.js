...
ProductManagement.prototype.get_product_by_category = function(provider_id, category, callback) {
    var collection = this.connection.collection('product');
    var searchSuccess = function(product) {
        return callback(false, product.ops);
    };
    ...
    collection.find({ $and: [{ provider_id: provider_id }, { category: category }] }).toArray(function(err, result) {
        if (err) {
            notification.message = "can not get";
            return callback(true, notification);
        } else {
